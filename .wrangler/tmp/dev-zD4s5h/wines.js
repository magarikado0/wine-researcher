...
var tastingLogs = sqliteTable("tasting_logs", {
  id: integer("id").primaryKey(),
  userId: text("user_id").notNull(),
  selectedTags: text("selected_tags"),
  // Storing as a JSON string or comma-separated values
    ...
    try {
      ...
      if (request.method === "POST") {
        ...
        const wineIds = [...new Set(
          searchResults.matches.map((match) => parseInt(match.id, 10))
        )];
        const db = drizzle(env.DB);
        const matchedWines = await db.select().from(wines).where(inArray(wines.id, wineIds));
        const orderedWines = wineIds.map(
          (id) => matchedWines.find((w) => w.id === id)
        ).filter(Boolean);
...
// .wrangler/tmp/bundle-27q4OQ/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default,
  middleware_miniflare3_json_error_default
];
var middleware_insertion_facade_default = wines_default;
...
// .wrangler/tmp/bundle-27q4OQ/middleware-loader.entry.ts
